...
The same argument applies inside an application. Keep concerns in separate modules — `default/hypr/`, `default/quickshell/modules/`, `config/nvim/lua/core/plugins_config/` — so a change touches one file and a reader looking for one behaviour finds one place. Splitting also decides how much an override costs: `~/.config/hypr/conf/local.lua` can replace one setting precisely because the defaults are loaded as separate modules that it runs after.
