...
- **Click opens somewhere to act on the card's subject.** Not more detail — somewhere to do something about it. Network is the worked example left in this section: its click opens `nmtui`, which is where you go to do something about either answer, and no tooltip could have been that. The Bell's history made the point better — it dismissed, cleared and toggled do-not-disturb, which is why it held keyboard focus — but that card is retired and what it opened is [the island](island/README.md)'s now. A card with nothing to act on gets no click handler, which is why the battery has none.
...
Nothing is built, and the problem that prompted it has since gone. The drawer came up because the notification history is anchored top right while the Bell that opens it sat at the bottom of a right-hand bar, so clicking the bell threw a panel diagonally across the screen. The bar has moved back to the top, and the bell is retired: what it opened is the island's expansion now, which grows out of the thing you clicked. The diagonal throw went with the layout that caused it.
