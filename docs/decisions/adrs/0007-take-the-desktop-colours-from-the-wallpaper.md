...
**The bar is tinted, not neutral.** A dark scheme's `background` is near black whatever the wallpaper, so a bar drawn on it showed no sign of the wallpaper at all. The bar strip is `tint`, matugen's `secondary_container`: coloured by the wallpaper, and still dark enough that the bar's text and status colours read on it. The secondary container rather than the primary one, so the accent on the focused workspace stays the loudest thing on the bar.
