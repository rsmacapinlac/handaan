...
- Code shape — shebangs, strict mode, indentation, quoting, the log helpers, and the palette file's shape — is [`docs/code-style.md`](docs/code-style.md). Read it before editing. "Match the file you are in" outranks everything else in it.
...
- `PopupAnchor.margins` does not move a popup. It applies to the anchor *rect*, so setting it to hold a tooltip off the bar changes nothing visible while the arithmetic reads as if it had -- a silent failure that survived three rounds of tuning the number. Reserve the gap as transparent window on the side facing the anchor instead, which is geometry you can measure. Where a popup needs to know where it actually landed, `itemRect(item)` gives the anchored item's bounds in the popup's own coordinates *after* placement, which is the only honest source once `PopupAdjustment` has had a say.
...
- Editing an existing QML file needs nothing: the unit leaves Quickshell's file watcher on, so a write under `default/quickshell` reloads the running shell. **Adding a QML type or singleton still needs a restart** before it exists -- the type registry is built once at launch and a hot reload does not rescan it, so the new type fails with `ReferenceError` while every file on disk is valid. `systemctl --user restart quickshell.service`. This is not a migration -- see [0006](docs/decisions/adrs/0006-present-handaan-dialogs-as-part-of-the-desktop.md).
- `default/quickshell/Commons/Colors.qml` is the raw palette, read live from the file `handaan-wallpaper-set` derives from the wallpaper (see [0007](docs/decisions/adrs/0007-take-the-desktop-colours-from-the-wallpaper.md)); `Commons/Theme.qml` maps it to semantic roles. Widgets bind to `Theme`, never to `Colors` directly, so a new wallpaper never touches widget code. Do not name a singleton `Palette` -- QtQuick defines that type already and wins the lookup silently, leaving every colour `undefined` instead of erroring.
