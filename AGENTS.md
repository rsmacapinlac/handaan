...
**The private repository's override mechanism is currently unbuilt.** It depended on `rcm` tree ordering, which handaan no longer uses. Until it is rebuilt on the copy model, do not assume a private file is shadowing a public default — check.
